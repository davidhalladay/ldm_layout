...
class SoftNet2NetTransformer(Net2NetTransformer):
    ...
    def sample(self, x, c, steps, temperature=1.0, sample=False, top_k=None,
        ...
        if self.pkeep <= 0.0:
            ...
            noise_shape = (x.shape[0], steps-1)
            #noise = torch.randint(self.transformer.config.vocab_size, noise_shape).to(x)
            noise = c.clone()[:,x_embed.shape[1]:-1]
            x = torch.cat((x,noise),dim=1)
            logits, _ = self.transformer(x, embeddings=x_embed, reverse_embed=True)
            # take all logits for now and scale by temp
            logits = logits / temperature
            # optionally crop probabilities to only the top k options
            if top_k is not None:
...
class MultiCBNet2NetTransformer(Net2NetTransformer):
    ...
    def forward(self, x, c):
        ...
        # differently because we are conditioning)
        target = z_embed
        # for i in range(z_indices.size(1)):
        #     target.append(z_indices[:, i, :]) 
        # make the prediction
        logits, _ = self.transformer(cz_indices[:, :-1])
        # cut off conditioning outputs - output i corresponds to p(z_i | z_{<i}, c)
    ...
    def log_images(self, batch, temperature=None, top_k=None, callback=None, lr_interface=False, is_test=False, **kwargs):
        ...
        # reconstruction
        # z_indices = z_indices.reshape(z_indices.shape[0], num_codebook, num_token_per_codebook).permute(0, 2, 1)
