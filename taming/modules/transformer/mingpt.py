...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, block_size, n_layer=12, n_head=8, n_embd=256, token_size=None,
                 embd_pdrop=0., resid_pdrop=0., attn_pdrop=0., n_unmasked=0, split_embed=0):
        super().__init__()
        if token_size is None: token_size = vocab_size
        config = GPTConfig(vocab_size=vocab_size, block_size=block_size, token_size=token_size,
        ...
        if split_embed:
            self.tok_emb = nn.Embedding(config.token_size, config.n_embd // split_embed)
        else:
            self.tok_emb = nn.Embedding(config.token_size, config.n_embd)
        ...
        if split_embed:
            ...
            for i in range(split_embed):
                self.head.append(nn.Linear(config.n_embd, config.vocab_size, bias=False))
        else:
    ...
    def forward(self, idx, embeddings=None, targets=None, reverse_embed=False):
        # forward the GPT model
        token_embeddings = self.tok_emb(idx) # each index maps to a (learnable) vector

        if self.split_embed:
            token_embeddings = token_embeddings.repeat(1, 1, self.split_embed)
        if embeddings is not None: # prepend explicit embeddings
            if reverse_embed:
                token_embeddings = torch.cat((token_embeddings, embeddings), dim=1)
            else:
        ...
        if type(self.head) is nn.ModuleList:
            logits = []
