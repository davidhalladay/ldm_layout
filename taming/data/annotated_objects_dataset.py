...
from taming.data.conditional_builder.objects_bbox import ObjectsBoundingBoxConditionalBuilder, ObjectsConditionalBuilder, CaptionsConditionalBuilder
...
class AnnotatedObjectsDataset(Dataset):
    ...
    def filter_categories(self) -> None:
        ...
        try:
            print('Filting appending categories')
            if self.category_allow_list:
                self.categories_append = {id_: cat for id_, cat in self.categories_append.items() if cat.name in self.category_allow_list}
            if self.category_mapping:
                self.categories_append = {id_: cat for id_, cat in self.categories_append.items() if cat.id not in self.category_mapping}
        except:
            pass
    ...
    def setup_category_id_and_number(self) -> None:
        ...
        try:
            print('Adding appending categories into main one.')
            self.category_ids_append = list(self.categories_append.keys())
            self.category_ids_append.sort()
            self.category_ids += self.category_ids_append
            self.categories = {**self.categories, **self.categories_append}
        except:
            pass
